...
```
                          ┌─────────────────────────────────────────────┐
                          │              Shared Postgres                │
                          │   (public metadata: id, user_id, metadata)  │
                          └──────────▲──────────────────────▲───────────┘
                          │          │                      │
                          │ write    │ read                 │ read
                          │          │                      │
  ┌──────────┐  upload_key│  ┌───────┴──────────┐    ┌──────┴──────────────┐
  │  Client A │─────────────►│                  │    │                     │
  └──────────┘  store       │   Storage Enclave │    │  Training Enclave   │
                            │  (secret-storage) │    │  (secret-consumer)  │
  ┌──────────┐  upload_key  │                  │    │                     │
  │  Client B │─────────────►│  /upload_key     │    │  /receive (keys)    │
  ...
  └──────────┘  store       │       │                          │
                            └───────┼──────────────────────────┘
                                    │
                                    │  PUT (encrypt)     GET (decrypt)
                                    ▼                          │
                          ┌────────────────────────────────────┴─────────┐
                          │           Tinfoil Buckets Sidecar              │
                          │       (S3 encryption proxy, same bucket)       │
                          └────────────────────┬──────────────────────────┘
                                               │
                                               ▼
                                    ┌─────────────────────┐
                          ...
                          Storage ──► /push ──► /receive (attested TLS)
                          ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─
```
...
- [confidential-secret-storage](../screen-pipe-poc/confidential-secret-storage) - storage enclave
- [confidential-debug-secret-consumer](../screen-pipe-poc/confidential-debug-secret-consumer) - training enclave
